...
- Makes use of great tensor libraries (`jax`, `tensorflow`, `PyTorch`, `PyTensor`) to do the maths
...
- `PyTensor` backend can compile to `jax`, `C`, `numba` (and more)
- Runs on GPU easily
- Best documentation
:::
:::
...
Basically, the Laplace Approximation (IN __LA__) means that we approximate the latent field with a Gaussian.
