...
ggplot() + geom_sf(data = tmp, aes(fill=SIR), col = NA) + scale_fill_viridis_c()
...
ggplot() + geom_sf(data = tmp, aes(fill=SIR), col = NA) + scale_fill_viridis_c() + theme_void() -> p1


SIR_CAT <- SIR

sir_leq0 <- 
  cut(SIR_CAT[SIR_CAT>0], breaks = quantile(SIR_CAT[SIR_CAT>0], 
...
ggplot() + geom_sf(data = tmp, aes(fill=SIR_CAT), col = NA) + scale_fill_viridis_d() + theme_void() -> p2
