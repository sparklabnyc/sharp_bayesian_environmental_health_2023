...
```{r}
data <- read_rds(here("data", "italy", "italy_mortality.rds")) |> ungroup()
glimpse(data)
summary(data)
```
...
Estimate the region-specific death rate/relative risk/excess?

ASK GARY WHAT EXPECTED IS. THEN WE CAN KNOW WHAT WE ARE ESTIMATING
Same as this I think: https://gkonstantinoudis.github.io/nimble/RmarkSLC.html
<!-- We also have a variable `expected` for the expected number of deaths. We can use this instead of modelling a mean. -->
...
\text{log}(\mu_i) = \text{log}(E_i) + \alpha + \theta\\
i=1,..., N \quad (observation)
$$
Parameter $\theta$ here is common for all observations $y_i$, regardless of what province the observation belongs to.
