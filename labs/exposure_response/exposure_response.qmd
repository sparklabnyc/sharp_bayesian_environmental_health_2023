...
3.  Linear exposure-response regression by region; and
...
BYMecoCode <- nimbleCode({
  ...
  RR.beta1_1NO2 <- exp(beta[1]) # / sd.no2) # get per 1 unit increase in the airpollution (scale back) ASK AKIS
...
```{r}
modelBYMeco.sim <- readRDS("NIMBLE_BYM_LINEAR")
```
...
Model description about piecewise linear. Now adjusted model to incorporate piecewise terms (TO DO)
...
BYMecoPWCode <- nimbleCode({
  ...
  RR.beta1_1NO2_low <- exp(beta[1])#/ sd.no2) # get per 1 unit increase in the air pollution for low concentrations (scale back) ASK AKIS
  RR.beta1_1NO2_high <- exp(beta[2])# / sd.no2) # get per 1 unit increase in the air pollution for high concentrations (scale back) ASK AKIS
...
Create data object as required for `NIMBLE`, this time incorporating a piecewise structure for $NO_2$.
...
```{r}
modelBYMPWeco.sim <- readRDS("NIMBLE_BYM_PWLINEAR")
```
...
# 3. Linear exposure-response regression by region

TO DO

# 4. Non-linear exposure-response regression

Linearity of and exposure response relationship is an assumption. It is also worth exploring whether an exposure-response relationship might be non-linear. Here, we will demonstrate that with a random walk of order 2, which is equivalent to a natural spline with equally-spaced knots.
...
BYMecoRW2Code <- nimbleCode({
  ...
  tau.b ~ dgamma(0.5, 0.0005) # prior on precison of random walk of order 2
...
```{r}
modelBYMRW2eco.sim <- readRDS("NIMBLE_BYM_RW2")
```
...
summarise_draws(exposure_posterior, default_summary_measures()) |>
  ...
  labs(x = "Normalised NO2", y = "log mortality risk")
```

We can also show the effect on the relative scale, relative to the risk at 0 concentration (TO FINISH)
...
#   t() |>
#   as_tibble() |>
#   mutate(temperature_bins = temperature_bins) |>
#   ggplot(aes(x = temperature_bins)) +
#   geom_line(aes(y = `50%`)) +
#   geom_ribbon(aes(ymin = `2.5%`, ymax = `97.5%`), fill = "grey80", alpha = 0.5) +
#   labs(x = "Temperature", y = "Relative mortality risk") +
#   lims(y = c(0.80, 1.4))
```
...
We fitted several models, some with either linear or non-linear models.
