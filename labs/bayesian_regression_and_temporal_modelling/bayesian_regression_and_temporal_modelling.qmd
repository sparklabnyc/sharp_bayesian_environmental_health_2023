...
This lab will explore some key temporal modelling concepts, including linear slopes, random walks and basic addition of linear exposure terms.
...
Let's plot the number of deaths by week over time
```{r}
...
::: aside
Crude death rates are simply total deaths divded by total population, without age-adjustment or anything else.
:::
...
Before running NIMBLE, extract month information from date.
```{r}
data_national = data_national |>
  ungroup() |>
