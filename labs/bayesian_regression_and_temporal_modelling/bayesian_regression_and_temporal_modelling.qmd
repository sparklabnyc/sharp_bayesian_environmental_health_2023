...
```{r}
mcmc_intervals(nimbleMCMC_samples_week_random_walk_more_samples, regex_pars = c("rw")) + coord_flip() +
...
rw_mean_value <- c()
for (i in seq(nrow(data_national))) {
  rw_new_mean_value <- mean(nimbleMCMC_samples_week_random_walk_with_temperature_by_month[, paste0("rw[", i, "]")])
  rw_mean_value <- c(rw_mean_value, rw_new_mean_value)
}

beta_temperature_mean_value <- c()
for (i in seq(12)) {
  beta_temperature_new_mean_value <- mean(nimbleMCMC_samples_week_random_walk_with_temperature_by_month[, paste0("beta_temperature_month[", i, "]")])
  beta_temperature_mean_value <- c(beta_temperature_mean_value, beta_temperature_new_mean_value)
}
beta_temperature_mean_value_df = data.frame(month=c(1:12),beta_temperature_mean_value)

beta_temperature_df = data.frame(month=data_national$month, weekly_t2m_anomaly=data_national$weekly_t2m_anomaly) |>
  left_join(beta_temperature_mean_value_df) |>
  mutate(beta_temperature_values = beta_temperature_mean_value * weekly_t2m_anomaly)

linear_month_temp_fit <- data_national |>
  mutate(
    .death_rate_fit = 100000 * exp(
      mean(nimbleMCMC_samples_week_random_walk_with_temperature_by_month[, "alpha"]) + beta_temperature_df$beta_temperature_values + rw_mean_value
    )
  ) |>
  mutate(residuals = rate - .death_rate_fit)
```
