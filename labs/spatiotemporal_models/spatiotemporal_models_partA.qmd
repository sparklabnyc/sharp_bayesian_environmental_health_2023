...
=======
We can do nicer plots using the package `ggplot2`. First let's reduce the resolution to get quicker maps.
>>>>>>> 82dd4944a258d683173652ad2922a4ffec74ecaf
...
```{r eval = TRUE}
ggsamples <- ggs(modelGS.sim$samples)
ggsamples
```
...
* Check convergence of resRR (Robbie: something gone wrong here)
```{r echo=TRUE, eval=TRUE}
ggsamplesbym <- ggs(modelBYM.sim$samples)
#<<<<<<< HEAD
#ggsamplesbym %>% filter(Parameter == c("resRR[1]", "resRR[19]", "resRR[41]", "resRR[72]")) %>%
#              ggs_traceplot() + theme_bw()
#=======
#ggsamplesbym %>%
#  filter(Parameter == c("RR[1]", "RR[19]", "RR[41]", "RR[72]")) %>%
#  ggs_traceplot() + theme_bw()
#>>>>>>> 82dd4944a258d683173652ad2922a4ffec74ecaf
```
...
* Which model parameters do you want to monitor? Set these before running NIMBLE Call this object *params*.
```{r echo=TRUE, eval=TRUE}
params <- c("sigma2.theta", "sigma2.phi", "overallRR", "theta", "beta1", "beta2", "RR.beta1", "RR.beta2", "resRR", "proba.resRR", "alpha")
```
...
```{r echo=TRUE, eval=TRUE}
ggsamplesbym <- ggs(modelBYMeco.sim$samples)

ggsamplesbym %>%
  filter(Parameter %in% c("alpha", "beta1", "beta2")) %>%
  ggs_traceplot() + theme_bw()
```
