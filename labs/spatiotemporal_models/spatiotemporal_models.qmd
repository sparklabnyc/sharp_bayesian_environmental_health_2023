...
We can do decent plots using the package `ggplot2`, which is very flexible and has lots of options for customising the way we want a plot to look. 

First let's reduce the resolution to get quicker maps.
...
And the covariates, including total ICU beds, NO2 and IMD:
```{r fig.width=11}
ggplot() +
...
The RRs will be smoothed using the Poisson model with a log-link function, as we have seen in previous lectures and labs. As usual in this workshop, the inference is done with `NIMBLE` called through `R.

In particular, let each area $i$ be indexed by  the integers $1, 2,...,N$. The model is as follows:
$$
\begin{eqnarray}
O_{i}  & \sim & \text{Pois}(\lambda_{i}E_{i} ) \quad i = 1,..., N \\ 
\log(\lambda_{i}) & = & \alpha + \theta_{i}  \\
\theta_{i} & \sim & N(0,\sigma_{\theta})
\end{eqnarray}
$$

where $\sigma_{\theta}$ is a standard deviation term that controls the magnitude of $\theta_{i}$. We will first write the model in `NIMBLE.` Specify the prior of $\tau_{\theta}$ in the code below. We can try a Gamma with parameters 1 and 0.01, which if you can remember is a sensible option for a Poisson count model as it goes from 0 upwards, which counts do:
```{r}
...
The following chunk of code subsets the data to London so the models are quicker to run
...
What are the parameters to be initialised? We have seen this stage many times previously. Create a list with two elements and call it `inits` (each a list) with different initial values for the parameters.
...
## A4. Ecological regression with the BYM model
...
* Check the convergence of the intercept and covariates NO2 and ICU beds. What do you observe?
...
Subsequently, we examined the effect of long term exposure to NO$_2$ on COVID-19 mortality. We fitted a BYM model to account for unknown spatial confounding but in addition we accounted for total number of ICU beds per LTLA. We reported evidence of an increased COVID-19 mortality for increasing levels of NO$_2$.
