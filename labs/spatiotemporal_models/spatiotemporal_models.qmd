...
And the covariates, including total ICU beds, NO$_2$ and quintile of Index of Multiple Deprivation (IMD):
```{r fig.width=11}
...
Two chains as described in the Bayesian workflow.
```{r}
inits <- list(
...
covid_data <- list(
  ...
  # covariates
  X1 = scale(data_england$TtlICUB)[, 1], # ICU beds
  X2 = data_england$NO2 # NO2
)
...
We fitted a BYM model to account for unknown spatial confounding but in addition we accounted for total number of ICU beds per LTLA.
